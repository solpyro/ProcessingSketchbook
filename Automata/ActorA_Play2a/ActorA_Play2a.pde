  ...
  for(int i=0;i<aActors.length-1;i++)
    for(int j=i+1;j<aActors.length;j++)
      if(dist(aActors[i].x,aActors[i].y,aActors[j].x,aActors[j].y)<=iProximity) {
        float x1 = aActors[i].x;
        float y1 = aActors[i].y;
        float x1b = aActors[i].x+(aActors[i].dx*aActors[i].r);
        float y1b = aActors[i].y+(aActors[i].dy*aActors[i].r);
        float x2 = aActors[j].x;
        float y2 = aActors[j].y;
        float x2b = aActors[j].x+(aActors[j].dx*aActors[j].r);
        float y2b = aActors[j].y+(aActors[j].dy*aActors[j].r);
        image.bezier(x1,y1,x1b,y1b,x2b,y2b,x2,y2);
      }
  image.endDraw();
...
void keyPressed() {
  switch(key) {
    case 'c':
      ...
      break;
    case 's':
      saveFrame("ActorA-Play2a-S"+iSeed+"-T"+(millis()-iRestart)+".png");
      break;
    case ' ':
